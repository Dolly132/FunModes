...
methodmap CrazyShop_mm
{
	...
	{
		OnClientPutInServer_CrazyShop(client);
	...
	{
		OnClientDisconnect_CrazyShop(client);
	...
	{
		OnTakeDamagePost_CrazyShop(victim, attacker, damage);
	...
	{
		OnTakeDamage_CrazyShop(victim, attacker, damage, result);
	...
	{
		OnPlayerRunCmdPost_CrazyShop(client, buttons, impulse);
...
#define LAST_MODE_TYPE CrazyShop_mm
...
int g_iCrazyShop_DamageRequired;
int g_iCrazyShop_CreditsReward;

bool g_bCrazyShop_SaveCredits;
bool g_bCrazyShop_DisableShop;
bool g_bCrazyShop_Enabled;
...
public void OnPluginStart_CrazyShop()
{
	...
	/* CONVARS */
	DECLARE_FM_CVAR(
		CRAZYSHOP_CONVAR_DAMAGE, "sm_crazyshop_damage",
		"5000", "The needed damage for humans to be rewarded with credits",
		("200,500,1000,1500,2000"), CONVAR_INT
	);
	THIS_MODE_INFO.cvars[CRAZYSHOP_CONVAR_DAMAGE].HookChange(CrazyShop_OnConVarChange);

	DECLARE_FM_CVAR(
		CRAZYSHOP_CONVAR_CREDITS, "sm_crazyshop_credits",
		"1", "How many credits to reward the human when they reach the needed damage?",
		("1,2,3,4,5"), CONVAR_INT
	);
	THIS_MODE_INFO.cvars[CRAZYSHOP_CONVAR_CREDITS].HookChange(CrazyShop_OnConVarChange);

	DECLARE_FM_CVAR(
		CRAZYSHOP_CONVAR_SAVECREDITS, "sm_crazyshop_savecredits",
		"1", "Save credits to a database or not",
		("0,1"), CONVAR_BOOL
	);
	THIS_MODE_INFO.cvars[CRAZYSHOP_CONVAR_SAVECREDITS].HookChange(CrazyShop_OnConVarChange);

	DECLARE_FM_CVAR(
		CRAZYSHOP_CONVAR_SLOWBEACON_RADIUS, "sm_crazyshop_slowbeacon_radius",
		"400.0", "Slow Beacon Radius",
		("300.0,400.0,500.0,600.0,700.0"), CONVAR_FLOAT
	);
	THIS_MODE_INFO.cvars[CRAZYSHOP_CONVAR_SLOWBEACON_RADIUS].HookChange(CrazyShop_OnConVarChange);

	DECLARE_FM_CVAR(
		CRAZYSHOP_CONVAR_DISABLE_SHOP, "sm_crazyshop_disable_shop",
		"0", "Enable/Disable the !crazyshop command",
		("0,1"), CONVAR_BOOL
	);
	THIS_MODE_INFO.cvars[CRAZYSHOP_CONVAR_DISABLE_SHOP].HookChange(CrazyShop_OnConVarChange);

	DECLARE_FM_CVAR(
		CRAZYSHOP_CONVAR_TOGGLE, "sm_crazyshop_enable",
		"1", "Enable/Disable CrazyShop Mode (This differs from turning it on/off)",
		("0,1"), CONVAR_BOOL
	);
...
public void InitCvarsValues_CrazyShop()
{
	...
	g_iCrazyShop_DamageRequired = _FUNMODES_CVAR_GET_VALUE(modeIndex, CRAZYSHOP_CONVAR_DAMAGE, Int);
	g_iCrazyShop_CreditsReward = _FUNMODES_CVAR_GET_VALUE(modeIndex, CRAZYSHOP_CONVAR_CREDITS, Int);

	g_bCrazyShop_SaveCredits = _FUNMODES_CVAR_GET_VALUE(modeIndex, CRAZYSHOP_CONVAR_SAVECREDITS, Bool);
	g_fCrazyShop_SlowBeaconRadius = _FUNMODES_CVAR_GET_VALUE(modeIndex, CRAZYSHOP_CONVAR_SLOWBEACON_RADIUS, Float);
	g_bCrazyShop_DisableShop = _FUNMODES_CVAR_GET_VALUE(modeIndex, CRAZYSHOP_CONVAR_DISABLE_SHOP, Bool);

	g_bCrazyShop_Enabled = _FUNMODES_CVAR_GET_VALUE(modeIndex, CRAZYSHOP_CONVAR_TOGGLE, Bool);
}

void CrazyShop_OnConVarChange(int modeIndex, int cvarIndex, const char[] oldValue, const char[] newValue)
{
	switch (cvarIndex)
	{
		case CRAZYSHOP_CONVAR_DAMAGE:
			g_iCrazyShop_DamageRequired = _FUNMODES_CVAR_GET_VALUE(modeIndex, cvarIndex, Int);

		case CRAZYSHOP_CONVAR_CREDITS:
			g_iCrazyShop_CreditsReward = _FUNMODES_CVAR_GET_VALUE(modeIndex, cvarIndex, Int);

		case CRAZYSHOP_CONVAR_SAVECREDITS:
			g_bCrazyShop_SaveCredits = _FUNMODES_CVAR_GET_VALUE(modeIndex, cvarIndex, Bool);

		case CRAZYSHOP_CONVAR_SLOWBEACON_RADIUS:
			g_fCrazyShop_SlowBeaconRadius = _FUNMODES_CVAR_GET_VALUE(modeIndex, cvarIndex, Float);

		case CRAZYSHOP_CONVAR_DISABLE_SHOP:
			g_bCrazyShop_DisableShop = _FUNMODES_CVAR_GET_VALUE(modeIndex, cvarIndex, Bool);

		case CRAZYSHOP_CONVAR_TOGGLE:
		{
...
public void OnTakeDamagePost_CrazyShop(int victim, int attacker, float damage)
{
	...
	if (DAMAGE_DEALT(attacker) >= g_iCrazyShop_DamageRequired)
	{
		int credits = g_iCrazyShop_CreditsReward;
		if (credits <= 0)
			return;
...
public Action Cmd_CrazyShopToggle(int client, int args)
{
	...
	if (THIS_MODE_INFO.isOn)
	{
		...
		if (g_bCrazyShop_SaveCredits && THIS_MODE_DB == null)
	...
	else
	{
		...
		for (int i = 1; i <= MaxClients; i++)
		{
			...
			OnClientDisconnect_CrazyShop(i);
		}
...
void CrazyShop_GetItems()
{
	...
	BuildPath(Path_SM, filePath, sizeof(filePath), "configs/FunModes/CrazyShop.cfg");
...
Action Timer_SlowBeaconRepeat(Handle timer, int userid)
{
	...
	float beaconRadius = g_fCrazyShop_SlowBeaconRadius;
