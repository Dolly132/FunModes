...
int[] GetRandomClientFromArray(int[] arr, int len)
{
	int result[2];
	int count;
	...
	result[1] = newArr[GetRandomInt(0, count - 1)];
	return result;
}
...
void RealityShift_AssignPlayers(bool saveOnly = false, bool random = true)
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		if (!random)
		{
			...
			RealityShift_SwapPositions(i, g_iRealityShiftAssigned);
		}
	}
	...
	{
		int result[2] = GetRandomClientFromArray(clients, count);
		clients[result[0]] = 0;
		
		int client = result[1];
		
		int result2[2] = GetRandomClientFromArray(clients, count);
		clients[result2[0]] = 0;
		
		int assignedTo = result2[1];
...
void RealityShift_SwapPositions(int client, int assignedTo)
{
	...
	TeleportEntity(target, clientOrigin);
