...
float g_fPullGame_TimerInterval;
float g_fPullGame_Speed;
float g_fPullGame_PullTime;
int g_iPullGame_RandomsZombies;
int g_iPullGame_RandomsHumans;
bool g_bPullGame_Enabled;
...
stock void OnPluginStart_PullGame()
{
	...
	DECLARE_FM_CVAR(
		PULLGAME_CONVAR_TIMER_INTERVAL, "sm_pullgame_timer_interval",
		"30.0", "After how many seconds to keep giving pull access to a random zm?",
		("15.0,30.0,40.0,60.0"), CONVAR_FLOAT
	);
	THIS_MODE_INFO.cvars[PULLGAME_CONVAR_TIMER_INTERVAL].HookChange(PullGame_OnConVarChange);

	DECLARE_FM_CVAR(
		PULLGAME_CONVAR_SPEED, "sm_pullgame_speed",
		"300.0", "Pulling Speed Value",
		("100.0,300.0,500.0,700.0,1000.0"), CONVAR_FLOAT
	);
	THIS_MODE_INFO.cvars[PULLGAME_CONVAR_SPEED].HookChange(PullGame_OnConVarChange);

	DECLARE_FM_CVAR(
		PULLGAME_CONVAR_PULL_TIME, "sm_pullgame_pull_time",
		"15.0", "Pulling Time",
		("10.0,15.0,30.0,40.0,55.0"), CONVAR_FLOAT
	);
	THIS_MODE_INFO.cvars[PULLGAME_CONVAR_PULL_TIME].HookChange(PullGame_OnConVarChange);

	DECLARE_FM_CVAR(
		PULLGAME_CONVAR_RANDOMS_HUMANS, "sm_pullgame_humans_count",
		"3", "How many humans to give pull access to?",
		("1,2,3,4,5,6,7"), CONVAR_INT
	);
	THIS_MODE_INFO.cvars[PULLGAME_CONVAR_RANDOMS_HUMANS].HookChange(PullGame_OnConVarChange);

	DECLARE_FM_CVAR(
		PULLGAME_CONVAR_RANDOMS_ZOMBIES, "sm_pullgame_zombies_count",
		"3", "How many zombies to give pull access to?",
		("1,2,3,4,5,6,7"), CONVAR_INT
	);
	THIS_MODE_INFO.cvars[PULLGAME_CONVAR_RANDOMS_ZOMBIES].HookChange(PullGame_OnConVarChange);

	DECLARE_FM_CVAR(
		PULLGAME_CONVAR_TOGGLE, "sm_pullgame_enable",
		"1", "Enable/Disable PullGame Mode (This differs from turning it on/off)",
		("0,1"), CONVAR_BOOL
	);
...
void InitCvarsValues_PullGame()
{
	...
	g_fPullGame_TimerInterval = _FUNMODES_CVAR_GET_VALUE(modeIndex, PULLGAME_CONVAR_TIMER_INTERVAL, Float);
	g_fPullGame_Speed = _FUNMODES_CVAR_GET_VALUE(modeIndex, PULLGAME_CONVAR_SPEED, Float);
	g_fPullGame_PullTime = _FUNMODES_CVAR_GET_VALUE(modeIndex, PULLGAME_CONVAR_PULL_TIME, Float);
	g_iPullGame_RandomsZombies = _FUNMODES_CVAR_GET_VALUE(modeIndex, PULLGAME_CONVAR_RANDOMS_ZOMBIES, Int);
	g_iPullGame_RandomsHumans = _FUNMODES_CVAR_GET_VALUE(modeIndex, PULLGAME_CONVAR_RANDOMS_HUMANS, Int);
	g_bPullGame_Enabled = _FUNMODES_CVAR_GET_VALUE(modeIndex, PULLGAME_CONVAR_TOGGLE, Bool);
}

void PullGame_OnConVarChange(int modeIndex, int cvarIndex, const char[] oldValue, const char[] newValue)
{
	switch (cvarIndex)
	{
		case PULLGAME_CONVAR_TIMER_INTERVAL:
			g_fPullGame_TimerInterval = _FUNMODES_CVAR_GET_VALUE(modeIndex, cvarIndex, Float);

		case PULLGAME_CONVAR_SPEED:
			g_fPullGame_Speed = _FUNMODES_CVAR_GET_VALUE(modeIndex, cvarIndex, Float);

		case PULLGAME_CONVAR_PULL_TIME:
			g_fPullGame_PullTime = _FUNMODES_CVAR_GET_VALUE(modeIndex, cvarIndex, Float);

		case PULLGAME_CONVAR_RANDOMS_ZOMBIES:
			g_iPullGame_RandomsZombies = _FUNMODES_CVAR_GET_VALUE(modeIndex, cvarIndex, Int);

		case PULLGAME_CONVAR_RANDOMS_HUMANS:
			g_iPullGame_RandomsHumans = _FUNMODES_CVAR_GET_VALUE(modeIndex, cvarIndex, Int);
...
public Action Cmd_PullGameToggle(int client, int args)
{
	...
	if (THIS_MODE_INFO.isOn)
	{
		...
		int humansCount = g_iPullGame_RandomsHumans;
		int zombiesCount = g_iPullGame_RandomsZombies;
		int interval = RoundToNearest(g_fPullGame_TimerInterval);
...
void PullGame_ToggleTimer(bool toggle)
{
	...
	if (toggle)
		g_hPullGameTimer = CreateTimer(g_fPullGame_TimerInterval, Timer_PullGame, _, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
}
...
Action Timer_PullGame(Handle timer)
{
	...
	int humansMaxCount = g_iPullGame_RandomsHumans;
	int zombiesMaxCount = g_iPullGame_RandomsZombies;
...
void PullGame_StartGrab(int client, int target)
{
	...
	CreateTimer(g_fPullGame_PullTime, Timer_PullGameFinish, client, TIMER_FLAG_NO_MAPCHANGE);
...
Action PullGame_Pull_Timer(Handle timer, int client)
{
	...
	if (distance > 40000.0)
	{
		...
		ScaleVector(velocity, g_fPullGame_Speed);
